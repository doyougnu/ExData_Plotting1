Plot1 <- function() {
  ...
  generatePlot1 <- function (dataframe) {
    #function takes filtered dataset and generates plot 1 for coursera course Exploratory Data Analysis, Project 1
    with(dataframe, hist(Global_active_power, col = "red", ylim = c(0, 1200), main = "Global Active Power", xlab = "Global Active Power (kilowatts)"))
  }
  ...
  #file isn't saved until dev.off() is called
  dev.off()
  generatePlot1(df)
}